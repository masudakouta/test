	...
	for (int nCntItem = 0; nCntItem < MAX_ITEM; nCntItem++, pItem++)
	{
		for (int nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
		{
			if (pItem->bUse == true)
			{
				...
				fLength = (g_aBullet[nCntBullet].pos.x - pItem->pos.x) * (g_aBullet[nCntBullet].pos.x - pItem->pos.x)
					+ (g_aBullet[nCntBullet].pos.y - pItem->pos.y) * (g_aBullet[nCntBullet].pos.y - pItem->pos.y)
					+ (g_aBullet[nCntBullet].pos.z - pItem->pos.z) * (g_aBullet[nCntBullet].pos.z - pItem->pos.z);
				if (fLength < (g_aBullet[nCntBullet].fRadius + pItem->fRadius) * (g_aBullet[nCntBullet].fRadius + pItem->fRadius))
				{
					// アイテム消去
					DeleteItem(nCntItem);

					SetExplosion(g_aBullet[nCntBullet].pos, 40.0f, 40.0f);
					//
					...
					//
					for (int i = 0; i < 100; i++, Box_Effect++)
					{
						SetBox_Effect(g_aBullet[nCntBullet].pos, Box_Effect->move, D3DXCOLOR(0.8f, 0.7f, 0.2f, 1.0f), 20.0f, 20.0f, 100);
					}
					// スコア加算
					ChangeScore(100);
		...
		if (g_aBullet[nCntBullet].bUse)
		{
			...
			else
			{
				...
				SetEffect(g_aBullet[nCntBullet].pos, D3DXVECTOR3(0.0f, 0.0f, 0.0f),
					D3DXCOLOR(0.2f, 0.2f, 0.2f, 0.50f), 16.0f, 16.0f, 30);
