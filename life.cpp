...
void UpdateLife(void)
{
	...
	//for (int nCntItem = 0; nCntItem < NUM_PLACE; nCntItem++, pItem++)
	//{
	//	if (pItem->bUse == true)
	//	{
	//		float fLength;
	...
	//			+ (pPlayer->pos.y - pItem->pos.y) * (pPlayer->pos.y - pItem->pos.y)
	//			+ (pPlayer->pos.z - pItem->pos.z) * (pPlayer->pos.z - pItem->pos.z);
	//		if (fLength < (pPlayer->fRadius + pItem->fRadius) * (pPlayer->fRadius + pItem->fRadius))
	//		{
	//			g_nLife--;
	//			if (g_nLife < 0)
	//			{
